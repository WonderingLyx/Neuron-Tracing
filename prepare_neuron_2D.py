...
#* python prepare_datasets_2D.py --datasets_name DRIVE --train_dataset_root_dir /mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/dataset/drive/
...
#* python prepare_datasets_2D.py --datasets_name ROAD --train_dataset_root_dir /mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/dataset/road/ 

def parse_args():
	parser = argparse.ArgumentParser()

	# (input dir) orginal data
	parser.add_argument('--datasets_name', default='Neron2d_big',help='datasets name') # CHASEDB1
	...
	# (output dir)
	parser.add_argument('--train_dataset_root_dir', default='/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Neron2d_big/dataset/',help='orginal centerline saved here')
	parser.add_argument('--N_patches', default=10000,help='Number of training image patches') # 150000
...
def prepare_train_datasets_2d(image_seq_dir, swc_tree_centerline_matirx, img_sim, label_sim, img_skl_distance, swc_tree_centerline_matirx_radius, swc_tree_centerline_matirx_vector,swc_tree_centerline_matirx_u_vector, BATCH_SHAPE, image_name, PATCH_NUM, seq_len=1, img_gap=1):
	...
	if img_sim.shape[0] == 1: #* 处理灰度图像
		img_sim = np.transpose(img_sim, [1,2,0])
		...
		for pos_id in range(pos_num):
			...
			for node_temp_id in node_id_list:
				...
				try:
					...
					raise ValueError(f"{image_seq_dir}")
...
def main_training_data(input_dir):
	...
	if datasets_name == 'DRIVE':
		...
		sample_gap = 1
		seq_len = 3
	elif datasets_name == "Neron2d":
		patch_num = args.N_patches // 20 // 4
		img_dir = org_image_train_dir + image_name + '.jpg'
		#img_mask_dir = org_mask_train_dir + image_name + '.tif'
		swc_dir = org_swc_train_dir + image_name + '.swc'
	...
	elif datasets_name == "Neron2d_big":
		patch_num = args.N_patches // 20 // 4
		img_dir = org_image_train_dir + image_name[:-len('_Probabilities')] + '.tif'
		#img_mask_dir = org_mask_train_dir + image_name + '.tif'
		swc_dir = org_swc_train_dir + image_name + '.swc'
	...
	prepare_train_datasets_2d(image_seq_dir, swc_tree_centerline_matirx, img_new, label_new, img_skl, swc_tree_centerline_matirx_radius, swc_tree_centerline_matirx_vector, swc_tree_centerline_matirx_u_vector, BATCH_SHAPE, image_name, patch_num, seq_len = seq_len, img_gap=sample_gap)
...
if __name__ == '__main__':
	...
	temp_swc_centerline_dir = args.image_dir + datasets_name + '/temp/swc_centerline/'
	temp_centerline_dir = args.image_dir + datasets_name + '/temp/centerline/'
	#TODO modify add temp mask dir
	#temp_mask_dir = args.image_dir + datasets_name + '/temp/mask/'
	...
	#TODO modify add temp mask dir
	# if not os.path.exists(temp_mask_dir):
	...
	org_label_list = glob.glob(org_label_train_dir + '*.tiff')
	...
	#main_training_data(org_label_list[5])

	org_label_list = glob.glob(org_label_test_dir + '*.tiff')
	org_label_num = len(org_label_list)
	print('find %d images' % (org_label_num))
	pool = mp.Pool(processes=cpu_core_num)  
	pool.map(main_test_data, org_label_list)
	#main_test_data(org_label_list[0])
