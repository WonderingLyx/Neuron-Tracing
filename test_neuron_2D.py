...
parser = argparse.ArgumentParser("test_neuron_2D")
...
parser.add_argument("--resize_radio", type=float, default=1.0)  # drive 2 15 same with neuron #!neuron big resize=1

parser.add_argument(
    "--pretrained_path",
    type=str,
    default="/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Results/Neuron2d_big/model/unet_model2d_0/15_32/epoch_15_batchsize_32_best.pth",
    help="path to pretrained model pth"
)
parser.add_argument(
    "--test_data_path",
    default="/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Neron2d_big/test/images",
    help="path to test imgs"
)
parser.add_argument(
    "--save_path",
    default="/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Results/Neuron2d_big/Predicts_unet",
...
class Model(nn.Module):
    def __init__(self, in_channel, out_channel, freeze_net=False, weights_path=None, device=None, device_ids=[0]):
        ...
        self.model = CSFL_Net_2D(in_channel, out_channel)
...
def test(args, device, device_ids=[0]):
    ...
    model = Model(3, 1, weights_path=args.pretrained_path, device=device, device_ids=device_ids)
    ...
    for test_img_dir in test_image:
        ...
        # img_new = cv2.resize(
        #     img_new,
        #     (round(img_new.shape[1]*resize_radio), round(img_new.shape[0]*resize_radio)),
        ...
        path = os.path.join(args.save_path, image_name + ".pro.lab.tif")
