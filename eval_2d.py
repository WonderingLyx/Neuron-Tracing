...
parser.add_argument(
    '--gt',
    type=str,
    default='/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Neron2d_big/test/labels',
...
def eval_two_imgs_maxpool(gt, predict, pool_kernel, device):
    ...
    pre = torch.nn.functional.max_pool2d(pre, kernel, 1, 0)
    label = torch.nn.functional.max_pool2d(label, kernel, 1, 0)
...
if __name__ == "__main__":
    ...
    # For Drive
    pred_list = [f for f in pred_list if 'lab' in f]
    ...
    df.to_excel(os.path.join(args.predicts, "Eval_results.xlsx"), index=False)
