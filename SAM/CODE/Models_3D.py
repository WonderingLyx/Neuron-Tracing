...
class SAM2_Net_2D(nn.Module):
	...
	def forward(self, x_, mode = 'train'):
		...
		feat_e5 = []
		batch_size, feat_c, d, h, w = x_.size()

		x_ = x_.reshape(batch_size, d, feat_c, h, w)
		...
		if mode == 'train':
			...
			for t in range(1, d): 
				...
				output_e5 = torch.cat([output_e5, e5_temp], dim=1)
			...
			return output_seg, output_centerline
...
class SAM2_Net_3D(nn.Module):
	def __init__(self, in_ch=1, out_ch=1, freeze_net = False, checkpoint_path=None, model_cfg=None, device=None,
		...
		self.model = SAM2_Net_2D(in_ch=in_ch, out_ch=out_ch, freeze_net = freeze_net, checkpoint_path=checkpoint_path, model_cfg=model_cfg, device=device)
	...
	def forward(self, x_, mode='train'):
		...
		batch_size, seq_len, c, d, h, w = x_.size()
		
		if mode == 'train':
			for t in range(seq_len): 
				input_image = x_[:, t, :, :, :, :]
				print()
				self.model(input_image, mode=mode)
		


if __name__ == "__main__":
	# trial_img = "/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/YiFu0729/dataset/training_datasets/2/1_pos_0/node_img.tif"
	# pretrain_path = "/mnt/40B2A1DBB2A1D5A6/lyx/project/MODEL/Sam2/sam2_hiera_large.pt"
	# device = torch.device("cuda:0")
	# img = tiff.imread(trial_img)

	# model = SAM2_Net_3D(1, 1, checkpoint_path=pretrain_path, device=device)
	# model = model.to(device)
	
	# img = torch.tensor(img)
	# #img = torch.permute(img, [0,3,1,2])
	# #img = func.resize(img, (352,352), interpolation=InterpolationMode.BICUBIC)
	# img = torch.reshape(img, (3,3,64,64,64))
	# img = img / 255
	# img = img.unsqueeze(0)
	# img = img.to(device)
	...
	#*** dataset ***
	dataset_path = "/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/YiFu0729/dataset/training_datasets/"
	pretrain_path = "/mnt/40B2A1DBB2A1D5A6/lyx/project/MODEL/Sam2/sam2_hiera_large.pt"
	device = torch.device("cuda:0")
	train_data_dir = dataset_path
	train_label_dir = dataset_path
	...
	Training_Data = Images_Dataset_folder(train_data_dir, train_label_dir, train_data_dir_list, train_label_dir_list, mode = 'train')
	x_img, y_lab, y_dis, y_r, y_exist, y1, y2, y3, y1_bin, y2_bin, y3_bin = Training_Data[0]
