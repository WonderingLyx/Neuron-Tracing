...
def train(args, model_name, device_ids, device):
    ...
    try:
        ...
        logger.info("Creation of the log directory '%s' failed " % LOG_DIR)
    else:
        logger.info("Successfully created the log directory '%s' " % LOG_DIR)
    writer = SummaryWriter(LOG_DIR)

    try:
        os.mkdir(MODEL_DIR)
    except OSError:
        logger.info("Creation of the model directory '%s' failed " % MODEL_DIR)
    else:
        logger.info("Successfully created the model directory '%s' " % MODEL_DIR)
