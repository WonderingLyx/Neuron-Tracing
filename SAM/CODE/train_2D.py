...
def train(args, model_name, device_ids, device):
    ...
    for i in range(epoch):
        ...
        for x_img, y_lab, y_dis, y_r, y_exist, y1, y2, y1_bin, y2_bin in train_loader:
            ...
            if not has_anomaly:
                print("所有参数梯度正常（无inf、nan，且未发现梯度过小）")
...
def inference_segmentation(args, model_name, device_ids, device):
    ...
    #     return model
    def model_unet_LSTM(model_input, in_channel, out_channel):
        model = model_input(in_channel, out_channel, checkpoint_path=args.sam2_pretrain)
        return model
    ...
    for test_img_dir in test_image:
        ...
        #* resize
        lab = shape_lab_image_new * img_mask_org
        lab = transform.resize(
            lab,
            output_shape=(1, shape[0], shape[1]),
            order=1,
            mode='edge',
            anti_aliasing=True
        )

        skl = shape_dis_image_new * img_mask_org
        skl = transform.resize(
            skl,
            output_shape=(1, shape[0], shape[1]),
            order=1,
            mode='edge',
        ...
        save_tif(lab, file_newname_lab, np.uint8)
        save_tif(skl, file_newname_skl, np.uint8)
