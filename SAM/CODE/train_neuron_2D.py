...
from tools.Losses import dice_loss, MSE_loss, L1_loss, dice_score, bce_loss_w, structure_loss
...
def train(args, model_name, device_ids, device):
    ...
    dt = datetime.today()
    log_name = (args.train_or_test + '_' + str(dt.date()) + "_" + str(dt.time().hour) + "_" +
                str(dt.time().minute) + "_" + str(dt.time().second)  
                )
    
    logger = Get_logger(str(args.model_save_dir) + str(args.gpu_id) + '/' + log_name + '.log')
    #######################################################
    #               load the data
    #######################################################
    ...
    def model_unet_LSTM(model_input, in_channel, out_channel, freeze_net):
        model = model_input(in_channel, out_channel, freeze_net, checkpoint_path=args.sam2_pretrain)
        return model
    ...
    softmax = torch.nn.Softmax(dim=-1)#.cuda(gpu)
    criterion = torch.nn.CrossEntropyLoss(reduction='mean')#.cuda(gpu)
    reg_criterion = torch.nn.MSELoss(reduction='mean')#.cuda(gpu)
    ...
    if to_restore:
        ...
        logger.info(read_model_path)
        if os.path.exists(read_model_path) and os.path.isdir(read_model_path):
            shutil.rmtree(read_model_path)
            logger.info('Model folder there, so deleted for newer one')
        try:
            os.mkdir(read_model_path)
        except OSError:
            logger.info("Creation of the model directory '%s' failed" % read_model_path)
        else:
            logger.info("Successfully created the model directory '%s' " % read_model_path)
    ...
    for i in range(epoch):
        ...
        for x_img, y_lab, y_dis, y_r, y_exist, y1, y2, y1_bin, y2_bin in train_loader:
            ...
            exist_id_rnn = np.where(y_exist_rnn==1) #* 选择出 pos 增强的数据
            ...
            if torch.isnan(x_img).any() or torch.isinf(x_img).any():
                logger.info("x_img输入图像数据有问题")
            if torch.isnan(y_lab).any() or torch.isinf(y_lab).any():
                logger.info("y_lab输入图像数据有问题")
            if torch.isnan(y_dis).any() or torch.isinf(y_dis).any():
                logger.info("y_dis输入图像数据有问题")

            if torch.isnan(y_exist_rnn).any() or torch.isinf(y_exist_rnn).any():
                logger.info("y_exist_rnn")
            if torch.isnan(y1_rnn).any() or torch.isinf(y1_rnn).any():
                logger.info("y1_rnn")
            if torch.isnan(y2_rnn).any() or torch.isinf(y2_rnn).any():
                logger.info("y2_rnn")
            if torch.isnan(y1_bin_rnn).any() or torch.isinf(y1_bin_rnn).any():
                logger.info("y1_bin_rnn")
            if torch.isnan(y2_bin_rnn).any() or torch.isinf(y2_bin_rnn).any():
                logger.info("y2_bin_rnn")
            ...
            for exist_id_num in exist_id_rnn[0]:
                ...
                y_d_class_pred_exist_rnn[exist_num][0] = sigmoid_layer(y_d_pred_1_rnn[exist_id_num]) #* sigmoid 对向量中的每一个元素进行操作,形成每一类的概率,注意概率之和不为1
            ...
            angle_result_norm_0 = torch.minimum(angle_result, torch.ones_like(angle_result) * 0.99) #* 0.99 这里是为了防止arccos函数梯度爆炸
            angle_result_norm_1 = torch.maximum(angle_result_norm_0, -torch.ones_like(angle_result) * 0.99)
            ...
            # Loss
            # loss_img_seg = lambda_seg * bce_loss_w(y_lab_pred, y_lab, 0.8) #*origin
            # loss_img_centerline = lambda_centerline * bce_loss_w(y_dis_pred, y_dis, 0.9)
            loss_img_seg = lambda_seg * structure_loss(y_lab_pred, y_lab) #* using old
            loss_img_centerline = lambda_centerline * structure_loss(y_dis_pred, y_dis)
            ...
            #! 损失函数检查
            lossT = loss_img_seg + loss_img_centerline + loss_img_radius_rnn + loss_img_direction_rnn_class + loss_img_direction_rnn_reg
            train_loss += lossT.item()
            lossT.backward()

            #! 梯度检查
            has_anomaly = False
            # 定义“梯度过小”的阈值（可根据实际情况调整，如1e-6、1e-8）
            grad_small_threshold = 1e-8  
            ...
            for name, param in model_train.named_parameters():
                ...
                if param.grad is None:
                    #print(f"参数 {name} 的梯度为 None（未参与反向传播）")
                    has_anomaly = True
                    continue
                
                # 检查梯度中的inf和nan（保留原有逻辑）
                has_inf = torch.isinf(param.grad).any().item()
                has_nan = torch.isnan(param.grad).any().item()
                
                if has_inf:
                    logger.info(f"参数 {name} 的梯度包含 inf")
                    has_anomaly = True
                if has_nan:
                    logger.info(f"参数 {name} 的梯度包含 nan")
                    has_anomaly = True
                ...
                # 检查梯度过小：两种维度
                # 1. 梯度中是否存在绝对值小于阈值的元素
                # 2. 梯度的L2范数（整体量级）是否小于阈值（反映整体梯度强度）
                # --------------------------
                grad_abs = torch.abs(current_grad)
                # 存在元素绝对值 < 阈值
                has_small_elements = (grad_abs < grad_small_threshold).any().item()
                # 计算梯度的L2范数（整体量级）
                grad_norm = torch.norm(current_grad, p=2).item()  # L2范数：sqrt(sum(grad^2))
                
                # 打印梯度过小的信息
                if has_small_elements or grad_norm < grad_small_threshold:
                    # print(f"参数 {name} 可能存在梯度过小：")
                    # print(f"  - 梯度绝对值最小值：{grad_abs.min().item():.8f}")
                    # print(f"  - 梯度L2范数（整体量级）：{grad_norm:.8f}")
                    # print(f"  - 阈值：{grad_small_threshold}\n")
                    has_anomaly = True

            if not has_anomaly:
                logger.info("所有参数梯度正常（无inf、nan，且未发现梯度过小）")
            ...
            if (train_step_temp+1) % 20 == 0:
                logger.info("===================================================================================")
                logger.info('Epoch: {}/{} \t Step: {}/{} \t Total Loss: {:.5f} \t Time: {:.5f}/step'.format(i + 1, epoch, train_step_temp, total_step, lossT.item(),  train_end_time-train_begin_time))
                
                if train_seg == False:
                    logger.info('Vector Angle LSTM: {:.5f} \t'.format(torch.mean(angle_arccos_rnn)))
                    logger.info('Radius LSTM Loss: {:.5f} \t Class LSTM Loss: {:.5f} \t Reg LSTM Loss: {:.5f} \t'.format(loss_img_radius_rnn.item(), loss_img_direction_rnn_class.item(), loss_img_direction_rnn_reg.item()))
                else:
                    logger.info('Segmentation Loss: {:.5f} \t F1: {:.5f} \t | Centerline Loss: {:.5f} \t F1: {:.5f} \t'.format(loss_img_seg.item(),lossdice_seg.item(), loss_img_centerline.item(), lossdice_centerline.item()))
                logger.info("-----------------------------------------------------------------------------------")

            train_step_temp += 1
            writer.add_scalar('Training_Segmentation_Loss', loss_img_seg.item(), global_step=global_step_)
            writer.add_scalar('Training_Centerline_Loss', loss_img_centerline.item(), global_step=global_step_)
            
            writer.add_scalar('Training_Radius_Reg_LSTM_Loss', loss_img_radius_rnn.item(), global_step=global_step_)
            writer.add_scalar('Training_Vector_Reg_LSTM_Loss', loss_img_direction_rnn_reg.item(), global_step=global_step_)
            writer.add_scalar('Training_Vector_Class_LSTM_Loss', loss_img_direction_rnn_class.item(), global_step=global_step_)
            writer.add_scalar('Training_Total_Loss', lossT.item(), global_step=global_step_)
            writer.add_scalar('Training_Total_Loss', lossT.item(), global_step=global_step_)
            global_step_ += 1
        ...
        logger.info("============================================")
        logger.info('total num: %d' % (split))
        logger.info("--------------------------------------------")
        logger.info('分割平均准确率:', np.mean(seg_acc_list))
        logger.info('中线平均准确率:', np.mean(centerline_acc_list))
        # R的计算误差
        logger.info("--------------------------------------------")
        logger.info('neg_num: %d neg_loss:%f' % (neg_num, neg_loss/(neg_num+1e-7)))
        logger.info('pos_num: %d pos_loss:%f' % (pos_num, pos_loss/(pos_num+1e-7)))
        logger.info('average_loss:%f' % ((neg_loss+pos_loss)/(neg_num+pos_num+1e-7)))
        # 分类的计算误差
        logger.info("--------------------------------------------")
        # print('越界平均准确率:', np.mean(class_acc_list0))
        # 角度的计算误差
        logger.info("--------------------------------------------")
        logger.info('acc1平均准确率:', np.mean(class_acc_list1))
        logger.info('acc2平均准确率:', np.mean(class_acc_list2))
        logger.info('平均偏差角度:', valid_reg_loss / valid_step_temp)
        logger.info("============================================")
        ...
        if (i+1) % 1 == 0:
            logger.info('Epoch: {}/{} \t Training Loss: {:.6f} \t Validation Loss: {:.6f} \t Class Loss: {:.6f} \t Reg Loss: {:.6f}'.format(i + 1, epoch, train_loss, valid_loss, valid_class_loss, valid_reg_loss))
        ...
        if r_loss <= r_loss_min: 
            logger.info('R loss decreased (%6f --> %6f).  Saving model ' % (r_loss_min, r_loss))
        ...
        logger.info('this epoch time: {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
        n_iter += 1
...
def inference_segmentation(args, model_name, device_ids, device):
    ...
    MODEL_DIR = str(args.model_save_dir) + str(args.gpu_id) + '/'
    checkpoint = torch.load(MODEL_DIR  + str(epoch) + '_' + str(batch_size) + '/epoch_' + str(epoch) + '_batchsize_' + str(batch_size) + '.pth', weights_only=True)
    ...
    test_image = glob.glob(TEST_DIR + '*.jpg')
    ...
    for test_img_dir in test_image:
        ...
        img_new = Image.open(test_img_dir).convert('L')
        ...
        #* 模拟3通道
        stack_org = np.stack([stack_org] * 3, axis=-1)
...
def inference_fastdeepbranchtracer(args, model_name, device_ids, device):
    ...
    for test_img_dir in test_image:
        ...
        # if image_name != '12':
        #     continue
        if image_name != 'Original_data_DIV5_WT-KO_shRNA_A3DIV5_A_002':
            continue
        # if image_name != '02R':
        ...
        stack_img = transform.resize(stack_img,
                                     (round(stack_img.shape[0]*resize_radio), round(stack_img.shape[1]*resize_radio)),
                                     order=3)
        stack_img = np.expand_dims(stack_img, axis=-1)
        stack_img = np.concatenate([stack_img] * 3, axis=-1)
        ...
        # #TODO 先尝试只有一个种子点
        input_seed = [[774],[352]]       
        seed_list_temp = [[416],[838]]
        seed_list = [[416],[838]]

        stack_seed_swc_dir = predict_seed_path + image_name + '.swc' # for drive
        stack_seed_swc_dir = "/mnt/40B2A1DBB2A1D5A6/lyx/TMP/neuron_tracing_from4090_1/Reference/DeepBranchTracer-new/DeepBranchTracer-main/data/Neron2d_big/test/swc/Original_data_DIV5_WT-KO_shRNA_A3DIV5_A_002_Probabilities.swc"
        ...
        for i in range(seed_list_flag.shape[0]):
            ...
            #* 从predict_exist 和 skl 找到对应节点的patch
